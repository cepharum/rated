...
$WRAPPER iptables -C INPUT -s "${1}" -j DROP &>/dev/null || \
	$WRAPPER iptables -A INPUT -s "${1}" -j DROP # 2>&1 | tee /var/log/rated/input.error.log

$WRAPPER iptables -C OUTPUT -d "${1}" -j REJECT &>/dev/null || \
	$WRAPPER iptables -A OUTPUT -d "${1}" -j REJECT # 2>&1 | tee /var/log/rated/output.error.log
